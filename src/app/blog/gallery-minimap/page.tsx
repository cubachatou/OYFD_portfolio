...
export default function GalleryMinimap() {
  ...
  useGSAP(() => {
    const imagesContainer = document.querySelector('[data-images-wrapper]') as HTMLElement;
  ...
  const handleClick = (index: number) => {
    const image = document.querySelector(`[data-images-wrapper] img:nth-child(${index + 1})`) as HTMLElement;
    ...
    <ReactLenis root>
      <main className="bg-black">
        <div className="relative w-full flex z-0">
          <div data-minimap className="absolute !z-5 top-[50vh] -translate-y-[calc(125px/2)] w-1/4 h-screen bg-black transition-colors duration-500">
            <div data-preview className="left-1/2 w-full flex flex-col items-center">
              {images.map((src, index) => (
                <div key={index} className="relative w-[100px] h-[125px] p-2.5 overflow-hidden">
                  <Image className="cursor-pointer" onClick={() => handleClick(index)} src={src} alt="" width={100} height={125} />
                </div>
              ))}
            </div>
          ...
          <div data-images-wrapper className="relative -z-[1] w-full flex flex-col items-center gap-[10vh] my-[5vh]">
            {images.map((src, index) => (
              <Image key={index} className="relative h-[90vh] w-[calc(90vh/1.5)] object-cover" src={src} alt="" width={564} height={846} />
            ))}
          </div>
        </div>
      </main>
